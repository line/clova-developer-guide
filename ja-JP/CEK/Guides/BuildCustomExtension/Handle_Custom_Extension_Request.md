...
![](/CEK/Assets/Images/CEK_Custom_Extension_Sequence_Diagram.png)

このようなユーザーのリクエストは一度で終わるリクエストの場合もありますが、次のように文脈が維持される必要のあるマルチターン対話の場合もあります。

![](/CEK/Assets/Images/CEK_Custom_Extension_Multi-turn_Sequence_Diagram.png)
...
<p><a href="/CEK/References/CEK_API.md#CustomExtEventRequest"><code>EventRequest</code></a>は、ユーザーの発話の有無に関わらず、デバイスの状態が変化したときにExtensionに送信されるメッセージです。これらのイベントは、デバイスの状態を取得したり、状態変化したことを検知することに利用できます。また、Extensionが<a href="/CEK/Guides/Build_Custom_Extension.md#ProvideAudioContent">オーディオコンテンツを提供する</a>際にも使用されます。ここでは、オーディオコンテンツ再生時の<code>EventRequest</code>については説明しません。</p>
</div>
